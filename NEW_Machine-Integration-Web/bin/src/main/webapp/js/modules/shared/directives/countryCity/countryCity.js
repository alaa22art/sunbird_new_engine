			...
			controller: ['$scope', 'lovService',
				function ($scope, lovService) {
					...
					$scope.assignCountry = function (selectedValue) {
						...
						if ($scope.model[$scope.options.city.name] && $scope.model[$scope.options.country.name] &&
							$scope.model[$scope.options.city.name].lkpCountry.rid === $scope.model[$scope.options.country.name].rid) {
							//do nothing
						} else {
							$scope.lkpCityOptions.clearLkps([$scope.lkpCityOptions]);
							$scope.assignCity(null);
						}
