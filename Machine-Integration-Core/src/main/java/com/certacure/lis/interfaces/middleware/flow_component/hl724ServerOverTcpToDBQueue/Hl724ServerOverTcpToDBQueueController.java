...
public class Hl724ServerOverTcpToDBQueueController extends FlowComponent<Hl724ServerOverTcpToDBQueueControllerConf> {
	...
	private Hl724ServerOverTcpToDBQueueController() {
			...
			if (httpRequstTransactionObj.getRequest_result_type().equals(REQUEST_RESULT_TYPE.SUCCUSS)
				...
				if (isSameDayOrder(httpRequstTransactionObj.getScheduleAppointment().getAppointmentDate())) {
					...
					strActivationResponse = "APPOINTMENT ["
							+ httpRequstTransactionObj.getScheduleAppointment().getAppointmentId() + ","
							+ httpRequstTransactionObj.getScheduleAppointment().getAppointmentCode()
							+ "] WITH DATE ["
	...
	private boolean isSameDayOrder(String strTransactionDate) throws Exception {
		...
		System.out.println("now day of month" + now.getDayOfMonth() + "now month" + now.getMonthValue() + "now year"
				+ now.getYear());
		System.out.println("visitDate day of month" + visitDate.getDayOfMonth() + "visitDate month"
				+ visitDate.getMonthValue() + "visitDate year" + visitDate.getYear());
		sameDay = now.getDayOfMonth() == visitDate.getDayOfMonth() && now.getMonthValue() == visitDate.getMonthValue()
