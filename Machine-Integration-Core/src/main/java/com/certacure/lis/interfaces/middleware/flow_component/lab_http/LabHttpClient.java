...
public class LabHttpClient extends FlowComponent<LabHttpClientConf> {
			...
			if (httpRequest.getValudation_result_type() == VALUDATION_RESULT_TYPE.FAILED
				...
				try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
					...
					default:
						httpRequest.setRequest_result_type(REQUEST_RESULT_TYPE.FAILED);
						httpRequest.getMessageTransaction().setIsValidated(true);
						httpRequest.getMessageTransaction().setIsSuccess(false);
					...
					httpRequest.getMessageTransaction().getIsValidated();
					httpRequest.getMessageTransaction().getIsSuccess();
					httpRequest.getMessageTransaction().getMessageType();
					httpRequest.getMessageTransaction().setNotes(httpRequest.getErrorDesc());
	...
	private void sendResult(LabResultMsg resultMsg) throws IOException {
			...
			if (!CollectionUtil.isCollectionEmpty(mappedMachineTestsList)) {
				...
				for (MachineTest machineTest : mappedMachineTestsList) {
					...
					if (machineOrder == null) {
						...
						String strError = "\n------------------------------------------------------------------------------------------------------------------------------------------------\n"
								+ "------------------------------------------------------------------------------------------------------------------------------------------------\n"
								+ " [ Mapping Error -992 ] : " + " Machine : " + machine.getName() + "\n"
								+ " Receive a Result with Sample ID : " + "\n" + resultMsg.order.container.specimenId
								+ "\n" + " And Test Code: " + "\n" + analysis.code + "\n"
								+ " no referance Order Exist or arrive from LIS side " + "\n"
								+ "\n----------------------------------------------------------------------------------------------------------------------------------------------------\n"
								+ "----------------------------------------------------------------------------------------------------------------------------------------------------\n";
					...
					if (isMatchedResultCode(analysis, machineTest)
						...
						if(resultInfo.getDataOrMeasurementValue().toString().trim().isEmpty())
						{
							continue;
						}
						...
						System.out.println("Added Result " + resultMsg.order.container.specimenId
								+ "  ################################################");
				...
				String strError = "\n------------------------------------------------------------------------------------------------------------------------------------------------\n"
						+ "------------------------------------------------------------------------------------------------------------------------------------------------\n"
						+ " [ Mapping Error -991 ] : " + " Machine : " + machine.getName() + "\n"
						+ " Receive a Result with Sample ID : " + "\n" + resultMsg.order.container.specimenId + "\n"
						+ " And Test Code: " + "\n" + analysis.code + "\n"
						+ " no referance Test Definition is Exist or mapped in the test catalog " + "\n"
						+ "\n----------------------------------------------------------------------------------------------------------------------------------------------------\n"
						+ "----------------------------------------------------------------------------------------------------------------------------------------------------\n";
		...
		if (!CollectionUtil.isCollectionEmpty(machineResultList)) 
		{
			...
			sendRecivedResultListToLIS(barcode, machineResultList);
		}
	 ...
	 * initiateObjects(); // try {
	 * 
	 * machineQuery = getQueryInfo(resultMsg); Order order = resultMsg.order;
	 * Container container = order.container; branch = getMachineBranch();
	 * 
	 * // Loop on the all tests received from the machine for (Analysis analysis :
	 * container.analyses) { // result map for the tests received from the machine
	 * to be prepared for sending // to the LIS resultMap = new HashMap<String,
	 * String>(); // object from MachineResult entity to be prepared for saving the
	 * result in the // MW DB. resultInfo = new MachineResult(); Result result =
	 * analysis.result; setResultValue(container, result);
	 * 
	 * List<MachineTest> mappedMachineTestsList =
	 * getMappedMachineTestByCode(analysis); // Update MachineOrder entity by
	 * setting result received based on // RequesterTestCode and SampleNo if
	 * (!CollectionUtil.isCollectionEmpty(mappedMachineTestsList)) {
	 * 
	 * for (MachineTest machineTest : mappedMachineTestsList) { machineOrder =
	 * getAssignedMachineOrder(resultMsg, machineTest.getTestCatalog());
	 * 
	 * if (machineOrder == null) {
	 * 
	 * String strNote = " [Error Sample Not Found] : " + "Sample no : " +
	 * resultMsg.order.container.specimenId + "Test : " + analysis.code +
	 * " not Arrived From LIS , check if sample is recived and accepted";
	 * 
	 * resultInfo.setNote(strNote);
	 * 
	 * messageTransaction = new MessageTransaction();
	 * 
	 ...
	 * 
	 * createMessageTransaction(resultInfo.toString(), machine,
	 * lkpMessageTransactionDirection, lkpMessageTransactionType,
	 * resultInfo.getSampleNo(), strNote, true, false, false, false);
	 * 
	 * break;
	 * 
	 * }
	 * 
	 * if (isMatchedResultCode(analysis, machineTest) && getResultType(container) ==
	 * RESULT_TYPE.PATIENT_RESULT) {
	 * 
	 * resultInfo = createMachineResultObj(normalActionCode, machineTest, container,
	 * analysis, result); MessageTransaction mt = messageTransactionService
	 * .getByID(resultMsg.order.getMessageTransactionID());
	 * resultInfo.setMessageTransaction(mt); machineResultList.add(resultInfo);
	 * 
	 * // QCOperationResult operationResult = //
	 * qcEventsDetailsService.qcNewPatientResult(resultInfo, machineTest);
	 * 
	 * } else if (machineOrder == null && getResultType(container) ==
	 ...
	 * resultInfo.setMessageTransaction(mt); machineResultList.add(resultInfo);
	 * 
	 * // qcEventsDetailsService.qcNewQCResult(resultInfo, machineTest, // machine);
	 * 
	 * } else if (machineOrder != null && getResultType(container) ==
	 * RESULT_TYPE.QUERY_RESULT) {
	 * 
	 ...
	 * 
	 * resultInfo = createMachineResultObj(normalActionCode, machineTest, container,
	 * analysis, result); MessageTransaction mt = new
	 * MessageTransaction(resultInfo.getMessageTransaction().getRid());
	 * resultInfo.setMessageTransaction(mt);
	 * 
	 * // resultInfo = getMachineResultDetailsObj(qualityActionCode, // machineTest,
	 * // container, analysis); addToMapList(container, result);
	 * machineResultList.add(resultInfo);
	 * 
	 * // qcEventsDetailsService.qcNewQCResult(resultInfo, machineTest, // machine);
	 * 
	 ...
	 * } else {
	 * 
	 * String strError = " [Error On Mapping Test] : " + " Machine : " +
	 * machine.getName() + "\n" + " Receive a Result with Sample ID : " + "\n" +
	 * resultMsg.order.container.specimenId + "\n" + " Test Code: " + "\n" +
	 * analysis.code + "\n" + " Result Code: " + "\n" + analysis.resultCode + "\n" +
	 * " no referance LIS order Exist or test not mapped " + "\n";
	 * 
	 * // if (runMode == RUN_MODE.DEBUG) { System.out.println(strError); // }
	 * 
	 * messageTransaction = new MessageTransaction();
	 * 
	 * CoreEventLog eventLog = getEventLogObject(strError);
	 ...
	 * createMessageTransaction(resultInfo.toString(), machine,
	 * lkpMessageTransactionDirection, lkpMessageTransactionType,
	 * resultInfo.getSampleNo(), strError, true, false, false, false);
	 * 
	 * // break; } }
	 * 
	 * if (!CollectionUtil.isCollectionEmpty(machineResultList)) { String barcode =
	 * machineResultList.get(0).getSampleNo(); MachineResultService
	 * machineResultService1;
	 * 
	 * machineResultService1 = (MachineResultService)
	 * SpringUtil.getBean("MachineResultService");
	 * 
	 * machineResultService1.addListResult(machineResultList);
	 * sendRecivedResultListToLIS(barcode, machineResultList); } // } catch
	 * (Exception ex) { // System.out.println("xxxx : " + ex.getMessage());
	 * 
	 * // messageTransaction = new MessageTransaction(); // } finally { //
	...
	private boolean isMatchedResultCode(Analysis analysis, MachineTest machineTest) {
		return analysis.resultCode.contains(machineTest.getResultCode());
	...
	private void sendRecivedResultListToLIS(String barcode, List<MachineResult> machineResultList2)
			throws IOException, JsonParseException, JsonMappingException {

		String resultAsJason = "";
		String apiResponse = "";
		int iCount = 0;

		for (MachineResult machineResult : machineResultList2) {
			resultAsJason += getJSONMessage(machineResult);

			if (iCount <= machineResultList2.size() - 1) {
				if (iCount == 0) {
					resultAsJason = "[" + resultAsJason;
				}
				if (iCount < machineResultList2.size() - 1) {
					resultAsJason += ",";
				}
		...
		if (branch != null && resultAsJason != "") {
			// String apiResponse = executePOST(resultAsJason,
			// branch.getIntegrationUrl(),branch.getIntegrationToken());
			apiResponse = executePost(resultAsJason, branch.getIntegrationUrl(), branch.getIntegrationToken());

			/*
			 * objectMapper = new ObjectMapper(); List<Map<String, String>> responseMap =
			 * objectMapper.readValue(apiResponse, new TypeReference<List<Map<String,
			 * String>>>() { });
			 */

			/*
			 * for (Map<String, String> mapResponse : responseMap) { boolean isAccepted =
			 * Boolean.parseBoolean(mapResponse.get("isAccepted")); if (isAccepted == false)
			 * { String output = MapUtil.mapToString(mapResponse); CoreEventLog coreEventLog
			 * = getEventLogObject(output);
			 * coreEventLogService.addCoreEventLog(coreEventLog); } }
			 */
			...
			if (apiResponse.equals("{}")) {
				...
				setLkpMessageTransactionType(MessageTransactionType.RESULT_TO_LIS);

				String strNote = "Result for sample : " + barcode + " and test : " + resultAsJason
						+ " was successfully sent to LIS ";

				createMessageTransaction(resultAsJason, machine, lkpMessageTransactionDirection,
						lkpMessageTransactionType, barcode, strNote, true, true, true, true);
				...
				String strNote = "Result for sample : " + barcode + " and test : " + resultAsJason
						+ " was not successfully sent to LIS !!! -->" + apiResponse;

				createMessageTransaction(resultAsJason, machine, lkpMessageTransactionDirection,
						lkpMessageTransactionType, barcode, strNote, false, true, true, false);
			}

		} else {
			String strError = " [Error On Sending Result ] : Sending error happed when sending data from LIS to MIW for "
					+ "\n" + " Machine : " + machine.getName() + "\n" + " For Order with Sample ID : " + "\n" + barcode
					+ "\n" + " For The Follwing Reason : " + apiResponse + "check dependencies reference if null "
					+ "\n";
			...
			createMessageTransaction(resultAsJason, machine, lkpMessageTransactionDirection, lkpMessageTransactionType,
					barcode, strError, true, false, true, false);
	...
	private String getJSONMessage(MachineResult machineResult) {
		return "{ " + "\"" + "result" + "\"" + ": " + "\"" + machineResult.getDataOrMeasurementValue() + "\"" + " , "
				+ "\"" + "testCode" + "\"" + " : " + "\"" + machineResult.getTestCode() + "\"" + " , " + "\""
				+ "barcode" + "\"" + " : " + "\"" + machineResult.getSampleNo() + "\"" + "}";
	}
	...
	private void setResultValue(Container container, Result result) {
		...
		int indexOfSubComp = result.value.indexOf('^');

		if (indexOfSubComp == -1) {
			resultInfo.setDataOrMeasurementValue(result.value);
		} else {
			resultInfo.setDataOrMeasurementValue(result.value.substring(indexOfSubComp + 1, result.value.length() - 1));
	...
	private void sendQuery(LabQueryMsg queryMsg) throws IOException {
		...
		try {
			...
			if (CollectionUtil.isCollectionEmpty(ordersList)) {
				...
				String strNote = "Query for sample : " + queryMsg.specimenIds
						+ " did not matched with any existed orders from LIS !! , check if sample is actually received in lab. ";

				createMessageTransaction(queryMsg.toString(), machine, lkpMessageTransactionDirection,
						lkpMessageTransactionType, queryMsg.specimenIds, strNote, false, false, false, false);
					...
					if (analysisListMapped.size() != 0) {
						...
						case ROCHE_COBAS_PRO_HL7_251 : 
							...
							container = new Container(
									queryMsg.specimenIds,
									analysisListMapped,
									queryMsg.strContainerRackNumber, 
									...
									queryMsg.queryTag, 
									queryMsg.queryMessageControlID,
									queryMsg.strBarcodeMode);
						...
						String strNote = "Mapping error for sample Number : " + queryMsg.specimenIds
								+ " No matching orders found in LIS! Sample received in the lab without correct machine/test mapping details. Please verify and update the necessary mapping information.";
						...
						if (analysisListMapped.size() == 0) {
							...
							case ABBOTT_ARCHITECT_CI4100:
								SendNegativeQuery(queryMsg, analysisListMapped, SpecimenDescriptor, machineType);
								break;
	...
	private void SendNegativeQuery(LabQueryMsg queryMsg, List<LabMessages.Analysis> analysisListMapped,
			String SpecimenDescriptor, MachineTypeEnum machineType) {
		LabOrderMsg orderMsg;
		Container container;
		container = new Container(queryMsg.specimenIds, 
				analysisListMapped,
				queryMsg.strContainerRackNumber,
				queryMsg.specimenPosition,
				null, 
		...
		Order order = new Order(patient, container, machine.getPassword(), machine.getName(), null, -1,queryMsg.queryMessageControlID,queryMsg.queryTag);
		orderMsg = new LabOrderMsg(order);
		conf.recipient.tell(orderMsg, self());
	}
	...
	private String sendErrorOnSelectionResponce(LabQueryMsg queryMsg) {
		String strError = "\n-----------------------------------------------------------------------------------------------------\n"
				...
				+ " [Error On Mapping Test] : " + " Machine : " + machine.getName() + "\n"
				+ " Recive Query With barcode : " + queryMsg.specimenIds + "\n"
	...
	private List<MachineOrder> getMachineOrders(LabQueryMsg queryMsg) {
		...
		// the both conditions
		  ...
		  String strBarcode = queryMsg.specimenIds.toString();
	        String strBarcode2 = strBarcode.replaceFirst("^0+", "");
		if (machine.getRequestTestWithNoResultOnly() == true || machine.getRequestNewTestOnly() == true) {
			lstOrder = machineOrderService.getOrderInformationBySampleNoWithOptions(strBarcode2,
					machine.getRequestTestWithNoResultOnly(), machine.getRequestNewTestOnly());
		} else {

			lstOrder = machineOrderService.getOrderInformationBySampleNo(strBarcode2,
	...
	private String executePost(String requestBody, String targetUrl, String API_KEY) {
		...
		try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
			...
			request = RequestBuilder.post().setUri(targetUrl)
					// .setHeader(HttpHeaders.ACCEPT, "application/application/json; charset=utf-8")
					.setHeader("API-KEY", "").setHeader(HttpHeaders.CONTENT_TYPE, "application/json; charset=utf-8")
			...
			return responseString + "--" + response.getStatusLine().getStatusCode();

		} catch (Exception ex) {
			return "ERROR ON SEND DATA TO LIS" + ex.getMessage();
		}
