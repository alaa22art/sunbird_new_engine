...
import com.certacure.lis.interfaces.middleware.enums.Enums.RESULT_TYPE;
import com.certacure.lis.interfaces.middleware.enums.Enums.RUN_MODE;
import com.certacure.lis.interfaces.middleware.enums.Enums.VALUDATION_RESULT_TYPE;
import com.certacure.lis.interfaces.middleware.interfaces.LabMessages;
...
import com.certacure.lis.interfaces.service.DataOutboundHL7MessageService;
import com.certacure.lis.interfaces.service.DataResultOutboundHL7MessageService;
...
public class LabHttpClient extends FlowComponent<LabHttpClientConf> {
	
	
	public class CALL_RESULT
	{
	    public int iErrorCode;
	    public String strErrorDetails;
	    public String strHttpResponse;
	...
	private boolean isSendingEmailEnabled2 = (config.getString("system.certacure.notification.khcc.email.enable")
			.equals("1")) ? true : false;
	...
	protected PartialFunction<Object, BoxedUnit> getBehaviour() {
		return ReceiveBuilder
				...
				.match(LabMachineMsg.class, conf -> {
				...
				}).match(DataInboundTempTable.class, jsonMessage -> {
					this.SendJsonMessage(jsonMessage);

				}).match(httpRequstTransaction.class, httpRequstTrans -> {
					this.sendHttpRequestToDestination(httpRequstTrans);
				})
				/*
				 * .match(httpRequstTransaction.class, httpRequstTrans -> {
	...
	public void initiateObjects() {
		...
		machineTypePanelService = (MachineTypePanelService) SpringUtil.getBean("MachineTypePanelService");
	...
	private void SendJsonMessage(DataInboundTempTable jsonMessage) {
		try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
			// Content-Type: application/json;charset=utf-8
			request = RequestBuilder.post().setUri("http://192.168.20.236:3322/Patients_Appointments")
					.setHeader(HttpHeaders.ACCEPT, "application/json;charset=utf-8")
					.setHeader("Authorization", "Basic UkNNQ0w6Njk2bGRpQXpLR3lXRkFU")
					// add request body
					.setEntity(new StringEntity(jsonMessage.getMessageBody().toString()))
	...
	public MachineQuery getQueryInfo(LabResultMsg resultMsg) {
		return mQueryService.getQueryBySampleAndMachine(resultMsg.order.container.specimenId.toString().trim(),
				machine);
	}

	private void sendHttpRequestToDestination(httpRequstTransaction httpRequestTransObj) throws Exception {
		...
		// Check if Transaction is DFT and With Duplicated RAD Order
		if (httpRequestTransObj.getMSG_Type().equals("DFT_P03_DUPICATED_RAD_ORDER_CORRECT")) {

			call_result = correctDuplicatedRadOrder(httpRequestTransObj);
			

			if (call_result.iErrorCode == 0) 
			{
				httpRequestTransObj.getMessageTransaction().setIsSuccuss(true);
				httpRequestTransObj.getMessageTransaction().setIsSent(true);
			...
			}else
			{
				...
				httpRequest.setErrorDesc("Error: " + "Error On Correcting Duplicated RAD Order : "
						+ httpRequest.getPostDetailFinancialTransaction().getVistaOrderID());

				throw new RuntimeException(httpRequest.getMessageTransaction().getNotes());
		...
		try {
			...
			if (httpRequest.getValudation_result_type() == VALUDATION_RESULT_TYPE.FAILED
					|| httpRequest.getValudation_result_type() == VALUDATION_RESULT_TYPE.UNKNOWNTYPE) {

				httpRequestTransObj.getMessageTransaction().setIsValidated(false);

				// do not send to RCM
				httpRequest.setRequest_result_type(REQUEST_RESULT_TYPE.FAILED);
				httpRequest.setErrorDesc("Error: " + httpRequest.getErrorDesc());
				...
				try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
					...
					case POST:
						request = RequestBuilder.post()
								.setUri(httpRequest.getAPI_URL_SUFFIX() + httpRequest.getAPI_URL_PREFIX())
								// .setHeader(HttpHeaders.ACCEPT, "application/application/json; charset=utf-8")
								...
								// add request body
								.setEntity(new StringEntity(httpRequestTransObj.getJSON(), "UTF-8"))
					...
					response.setEntity(entity);
					String responseString = EntityUtils.toString(entity, "UTF-8");
					/////// httpRequest.getMessageTransaction().setNotes(responseString);
					...
					httpRequest.getMessageTransaction().setResponse(responseString);
					httpRequest.getMessageTransaction().setNotes(response.toString());
					...
					case SUCCUSS:
						...
						if (httpRequestTransObj.getMSG_Type().equals("DFT_P03")) {
							...
							JsonNode identifierNode = jsonNode.path("payload").path("identifier").path("value");
							JsonNode orderItemsArray = jsonNode.path("payload").path("orderItems");
							JsonNode encounterNode = jsonNode.path("payload").path("encounter").path("value");
							...
							if (actionIDNode != null && identifierNode != null && encounterNode != null) {
								...
								httpRequest.getPostDetailFinancialTransaction().setOrder_by_id(certa_id);
								httpRequest.getPostDetailFinancialTransaction().setRCMOrderActionID(action_id);
								httpRequest.getPostDetailFinancialTransaction().setEncounter(encounter);
								httpRequest.getPostDetailFinancialTransaction()
										.setAppointmentID(httpRequest.getAppointmentID());
								postDetailFinancialTransactionService
										.add(httpRequest.getPostDetailFinancialTransaction());

							}
						} else if (httpRequestTransObj.getMSG_Type().equals("ADT")) {

						}
						// else if (httpRequestTransObj.getMSG_Type().equals("SIU_INTERNAL_UPDATE"))
						// {
						/*
						 * int iErrorCode = -1;
						 * 
						 * String strUpdateVisitAPIURL = httpRequestTransObj.getAPI_URL_PREFIX() +
						 * String.format(API_URL_PREFEX.VISIT_INTERNAL_UPDATE.toString(),
						 * httpRequestTransObj.getAppointmentID()) ;
						 * 
						 * iErrorCode = updatedVisitInfoByAppointment( strUpdateVisitAPIURL,
						 * httpRequestTransObj, HTTP_REQUESTED_METHOD_TYPE.PATCH);
						 * 
						 * String strUpdateOrderAPIURL = httpRequestTransObj.getAPI_URL_PREFIX() +
						 * String.format(API_URL_PREFEX.ORDER_INTERNAL_UPDATE.toString(),
						 * httpRequestTransObj .getPostDetailFinancialTransaction().getVistaOrderID()) ;
						 * 
						 * iErrorCode = updatedOrderInfoByAppointment( strUpdateOrderAPIURL,
						 * httpRequestTransObj, HTTP_REQUESTED_METHOD_TYPE.PATCH);
						 */
					...
					default:
						...
						httpRequest.getMessageTransaction().setIsSent(false);
						// httpclient.close();
						throw new RuntimeException(httpRequest.getErrorDesc());
					...
					/*
					 * if (strError.length > 2) { httpRequest .setErrorDesc("Error:" +
					...
					httpRequest.setRequest_result_type(REQUEST_RESULT_TYPE.FAILED);
					httpRequest.setErrorDesc("Error:" + ex.getMessage());
					httpRequest.getMessageTransaction().setNotes(httpRequest.getErrorDesc());
					...
					httpRequest.getMessageTransaction().getMessageType();
					httpRequest.getMessageTransaction().setNotes(httpRequest.getErrorDesc());
					httpRequest.getMessageTransaction().getPatientID();
					httpRequest.getMessageTransaction().getNationalID();

					AddEmailError(httpRequest, ex);
			...
			if (!httpRequest.getMessageTransaction().getAdtOperationType().equals("DFT_PF")) {
				AddEmailError(httpRequest, ex);
			}

		} finally {

			messageTransactionService.addMessageTransaction(httpRequest.getMessageTransaction().getMachine(),
					httpRequest.getJSON(), httpRequest.getMessageTransaction().getMessageDirection(),
					httpRequest.getMessageTransaction().getMessageType(),
					httpRequest.getMessageTransaction().getIsSuccuss(),
					httpRequest.getMessageTransaction().getIsValidated(),
					httpRequest.getMessageTransaction().getIsSent(), httpRequest.getMessageTransaction().getNotes(),
					"NO ACK CREATED", httpRequest.getMessageTransaction().getMessageControlID(),
					httpRequest.getMessageTransaction().getPatientID(),
					httpRequest.getMessageTransaction().getNationalID(),
					httpRequest.getMessageTransaction().getResponse(),
					httpRequest.getMessageTransaction().getAdtOperationType());
	...
	private CALL_RESULT correctDuplicatedRadOrder(httpRequstTransaction httpRequestTransObj) throws Exception {

		CALL_RESULT callResult = new CALL_RESULT();

		try {
			
				String strUpdateVisitAPIURL = httpRequestTransObj.getAPI_URL_SUFFIX() + String.format(
						API_URL_PREFEX.VISIT_INTERNAL_UPDATE.getValue(), httpRequestTransObj.getOldAppointmentID());

				callResult = updatedVisitInfo(strUpdateVisitAPIURL, httpRequestTransObj,
						HTTP_REQUESTED_METHOD_TYPE.PATCH);
				
				if(callResult.iErrorCode == -1)
				{
					httpRequestTransObj.setErrorDesc("Duplicate RAD Error - appointment update");
					httpRequestTransObj.getMessageTransaction().setIsValidated(true);
					httpRequestTransObj.getMessageTransaction().setIsSent(true);
					httpRequestTransObj.getMessageTransaction().setNotes("Duplicate RAD Error : Replace appointment with id =  " + httpRequestTransObj.getOldAppointmentID()+ " with new App id = " +httpRequestTransObj.getAppointmentID()+
							", has not ateemped succussfuly , " + "Reason : "
					+ callResult.strHttpResponse);
					
					throw new RuntimeException(httpRequestTransObj.getMessageTransaction().getNotes());
				}
				
			

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if (callResult.iErrorCode == 0) {
				String strUpdateOrderAPIURL = httpRequestTransObj.getAPI_URL_SUFFIX()
						+ String.format(API_URL_PREFEX.ORDER_INTERNAL_UPDATE.getValue(),
								httpRequestTransObj.getPostDetailFinancialTransaction().getVistaOrderID());

				callResult = updatedOrderInfo(strUpdateOrderAPIURL, httpRequestTransObj,
						HTTP_REQUESTED_METHOD_TYPE.PATCH);
			}

			if (callResult.iErrorCode == 0) {
				// update old DFT order by new appointment
				List<PostDetailFinancialTransaction> listPDFT = postDetailFinancialTransactionService
						.getOrdersByAppointmentAndId(httpRequestTransObj.getOldAppointmentID(),
								httpRequestTransObj.getPostDetailFinancialTransaction().getVistaOrderID());

				if (listPDFT.size() > 0) {
					PostDetailFinancialTransaction post = listPDFT.get(0);
					listPDFT.get(0).setAppointmentID(httpRequestTransObj.getAppointmentID());
					postDetailFinancialTransactionService.add(listPDFT.get(0));
		...
		} catch (Exception ex)
		{
			...
			messageTransactionService.addMessageTransaction(httpRequestTransObj.getMessageTransaction().getMachine(),
					httpRequestTransObj.getJSON(), httpRequestTransObj.getMessageTransaction().getMessageDirection(),
					httpRequestTransObj.getMessageTransaction().getMessageType(),
					httpRequestTransObj.getMessageTransaction().getIsSuccuss(),
					httpRequestTransObj.getMessageTransaction().getIsValidated(),
					httpRequestTransObj.getMessageTransaction().getIsSent(), httpRequestTransObj.getMessageTransaction().getNotes(),
					"NO ACK CREATED", httpRequestTransObj.getMessageTransaction().getMessageControlID(),
					httpRequestTransObj.getMessageTransaction().getPatientID(),
					httpRequestTransObj.getMessageTransaction().getNationalID(),
					httpRequestTransObj.getMessageTransaction().getResponse(),
					httpRequestTransObj.getMessageTransaction().getAdtOperationType());
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	private CALL_RESULT updatedAppointmentInfo(String strUpdateAppointmentAPIURL, httpRequstTransaction httpRequestTransObj,
			HTTP_REQUESTED_METHOD_TYPE methodType) throws Exception {
		CALL_RESULT call_result = new CALL_RESULT();

		String strUpdateAppointmentJSON = createUpdateAppointmentJSON(httpRequestTransObj);

		

		try {
			call_result = callApi(strUpdateAppointmentAPIURL, httpRequestTransObj,
					strUpdateAppointmentJSON, methodType);		
	...
	private String createUpdateAppointmentJSON(httpRequstTransaction httpRequestTransObj) throws Exception {

		String strRequest;

		strRequest = "{" + "\"" + "newIdentifier" + "\"" + ":" + "{" + "\"" + "value" + "\"" + ":" + "\""
				+ httpRequestTransObj.getAppointmentID() + "\"" + "}," + "\"" + "start" + "\"" + ":" + "\""
				+ getDateTimeISO8601(httpRequestTransObj.getPostDetailFinancialTransaction().getTransactionDate())
				+ "\"" + "," + "\"" + "end" + "\"" + ":" + "\""
				+ getEndTime(httpRequestTransObj.getPostDetailFinancialTransaction().getTransactionDate()) + "\"" + ","
				+ "\"" + "facility" + "\"" + ":" + "{" + "\"" + "code" + "\"" + ":" + "{" + "\"" + "value" + "\"" + ":"
				+ "\"" + "KHCC" + "\"" + "}}," + "\"" + "recordedBy" + "\"" + " : " + "{" + "\"" + "code" + "\"" + " : "
				+ "{" + "\"" + "value" + "\"" + " : " + "\"" + "2116" + "\"" + "}}" + "," + "\"" + "recordedDate" + "\""
				+ " : " + "\""
				+ httpRequestTransObj.getRecivedDateTime()
				+ "\"" + "}";
	...
	private CALL_RESULT updatedOrderInfo(String strURL, httpRequstTransaction httpTransOb,
			HTTP_REQUESTED_METHOD_TYPE methodType) throws Exception {
		CALL_RESULT call_result = new CALL_RESULT();

		/*
		 * { "authoredOn": "2024-07-30T08:28:08.653Z", "facility": { "code": { "value":
		 * "string" } }, "recordedBy": { "code": { "value": "string" } },
		 * "recordedDate": "2024-07-30T08:28:08.653Z" }
		 */

		String strRequest ="{"
				+ "\""
				+ "authoredOn" + "\"" + " : " + "\"" + getDateTimeISO8601( httpTransOb.getPostDetailFinancialTransaction().getTransactionDate().toString())+ "\"" + ","
				+"\"" + "facility" + "\"" + ":" + "{" + "\"" + "code" + "\"" + ":" + "{" + "\"" + "value"
				+ "\"" + ":" + "\"" + "KHCC" + "\"" + "}}," + "\"" + "recordedBy" + "\"" + " : " + "{" + "\"" + "code"
				+ "\"" + " : " + "{" + "\"" + "value" + "\"" + " : " + "\"" + "1" + "\"" + "}}," + "\""
		...
		try {
			call_result = callApi(strURL, httpTransOb, strRequest, HTTP_REQUESTED_METHOD_TYPE.PATCH);

		} catch (Exception ex) {
			call_result.iErrorCode = -1;
	...
	private CALL_RESULT updatedVisitInfo(String strURL, httpRequstTransaction httpTransObj,
			HTTP_REQUESTED_METHOD_TYPE methodType) {
		CALL_RESULT call_result = new CALL_RESULT();
		try {
			
			
		String strRequest = 
				"{"+"\"" + "rescheduledAppointment" + "\"" + ":" + "{" + "\"" + "identifier" + "\"" + ":" + "{" + "\"" + "value" 
				+ "\"" + ":" + "\"" + httpTransObj.getAppointmentID()+ "\"" + "}}," 
				+"\"" + "facility" + "\"" + ":" + "{" + "\"" + "code" + "\"" + ":" + "{" + "\"" + "value"
				+ "\"" + ":" + "\"" + "KHCC" + "\"" + "}}," 
				+ "\"" + "recordedBy" + "\"" + " : " + "{" + "\"" + "code"
				+ "\"" + " : " + "{" + "\"" + "value" + "\"" + " : " + "\"" + "1" + "\"" + "}}" +  ","
				...
				+ getCurrentDateTime()+ "\"" + ","
				+  "\"" + "visitDate" + "\"" + " : " + "\""
				+ getDateTimeISO8601(httpTransObj.getPostDetailFinancialTransaction().getTransactionDate().toString()) + "\"" + "}";

		
			call_result = callApi(strURL, httpTransObj, strRequest, methodType);
	...
	private CALL_RESULT callApi(String strURL, httpRequstTransaction httpRequst, String strJSON_OBJ, HTTP_REQUESTED_METHOD_TYPE methodType) {
		...
		try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
			...
			switch (methodType) {
			case POST:
				request = RequestBuilder.post().setUri(strURL)
				

						.setHeader("API-KEY", httpRequst.getAPI_AUTH_KEY())
						.setHeader(HttpHeaders.CONTENT_TYPE, "application/json; charset=utf-8")
						.setHeader(HttpHeaders.ACCEPT, "application/json")
						.setEntity(new StringEntity(strJSON_OBJ, "UTF-8")).build();
				break;
			case PATCH:
				request = RequestBuilder.patch().setUri(strURL)
			...
			response.setEntity(entity);
			String responseString = EntityUtils.toString(entity, "UTF-8");
			call_result.strHttpResponse = responseString;
			call_result.strJson = strJSON_OBJ;
			...
			AddMessageTransactionLog(httpRequst, strJSON_OBJ, call_result, response, responseString);
	...
	private void AddMessageTransactionLog(httpRequstTransaction httpRequst, String strJSON_OBJ, CALL_RESULT call_result,
			CloseableHttpResponse response, String responseString) {
		httpRequstTransaction newHttpRequest = new 
				httpRequstTransaction();
		
		newHttpRequest = httpRequst;
		
			
		if (response.getStatusLine().getStatusCode() == 200)
		{
			
			call_result.iErrorCode = 0;
			
			newHttpRequest.getMessageTransaction().setIsSuccuss(true);
			newHttpRequest.getMessageTransaction().setIsSent(true);
			newHttpRequest.getMessageTransaction().setIsValidated(true);
			newHttpRequest.setJSON(strJSON_OBJ);
			newHttpRequest.getMessageTransaction().setNotes(responseString);
			newHttpRequest.getMessageTransaction().setResponse(response.toString());
		...
		}else
		{
			...
			newHttpRequest.getMessageTransaction().setIsSuccuss(false);
			newHttpRequest.getMessageTransaction().setIsSent(false);
			newHttpRequest.getMessageTransaction().setIsValidated(false);
			newHttpRequest.setJSON(strJSON_OBJ);
			newHttpRequest.getMessageTransaction().setNotes(responseString);
			newHttpRequest.getMessageTransaction().setResponse(response.toString());
		...
		messageTransactionService.addMessageTransaction(
				newHttpRequest.getMessageTransaction().getMachine(),
				newHttpRequest.getJSON(), 
				newHttpRequest.getMessageTransaction().getMessageDirection(),
				newHttpRequest.getMessageTransaction().getMessageType(),
				newHttpRequest.getMessageTransaction().getIsSuccuss(),
				newHttpRequest.getMessageTransaction().getIsValidated(),
				newHttpRequest.getMessageTransaction().getIsSent(),
				newHttpRequest.getMessageTransaction().getNotes(), response.getStatusLine().toString(),
				newHttpRequest.getMessageTransaction().getMessageControlID(),
				newHttpRequest.getMessageTransaction().getPatientID(),
				newHttpRequest.getMessageTransaction().getNationalID(),
				newHttpRequest.getMessageTransaction().getResponse(),
				newHttpRequest.getMessageTransaction().getAdtOperationType());
	}

	private void AddEmailError(httpRequstTransaction httpRequest, Exception ex) {
		if (isSendingEmailEnabled2) {
			OutboundErrorEmailEntity outboundErrorEmailEntity = new OutboundErrorEmailEntity();
			outboundErrorEmailEntity.setBody(ex.getMessage());
			outboundErrorEmailService = (OutboundErrorEmailService) SpringUtil.getBean("OutboundErrorEmailService");
			outboundErrorEmailEntity.setMessageControlID(httpRequest.getMessageTransaction().getMessageControlID());
			outboundErrorEmailEntity.setMessageText(httpRequest.getMessageTransaction().getMessageBody());
			outboundErrorEmailEntity.setPatientId(httpRequest.getMessageTransaction().getPatientID());
			outboundErrorEmailEntity.setadtOperationType(httpRequest.getMessageTransaction().getAdtOperationType());
			outboundErrorEmailEntity.setAdmissionNo(httpRequest.getAdmissionNumber());
			outboundErrorEmailEntity.setBody(httpRequest.getMessageTransaction().getNotes());
			...
			outboundErrorEmailEntity.setTenantId(115l);
			outboundErrorEmailEntity.setBranchId(1l);
			outboundErrorEmailEntity.setCreatedBy(1l);

			outboundErrorEmailService.save(outboundErrorEmailEntity);
		}
	}
			...
			if (!CollectionUtil.isCollectionEmpty(mappedMachineTestsList)) {
				...
				for (MachineTest machineTest : mappedMachineTestsList) {
					...
					if (machineOrder == null) {
						resultInfo.setNote("not matched order for Sample no : " + resultMsg.order.container.specimenId
								+ "and " + "host code : " + machineTest.getHostCode());
					}

					if (isMatchedResultCode(analysis, machineTest)
							&& getResultType(container) == RESULT_TYPE.PATIENT_RESULT) {

						resultInfo = createMachineResultObj(normalActionCode, machineTest, container, analysis, result);
					...
					} else if (machineOrder == null && getResultType(container) == RESULT_TYPE.QUERY_RESULT) {
						...
						resultInfo = createMachineResultObj(normalActionCode, machineTest, container, analysis, result);
						MessageTransaction mt = new MessageTransaction(resultInfo.getMessageTransaction().getRid());
						resultInfo.setMessageTransaction(mt);
	...
	private MachineResult createMachineResultObj(LkpOrderActionCode actionCode, MachineTest machineTest,
			Container container,
	...
	private DataResultOutboundHL7Message initiateResultOutboundObj(MachineResult resultInfo, String messageHL7) {
	...
	private MachineOrder getAssignedMachineOrder(LabResultMsg resultMsg, TestCatalog selectedTestCatalog) {
	...
	private MachineOrder getMachineOrder(LabResultMsg resultMsg, TestCatalog selectedTestCatalog) {
		return machineOrderService.getBySampleAndTestCode(resultMsg.order.container.specimenId.trim(),
				selectedTestCatalog.getRequesterTestCode(), machine.getTenantId(), machine.getBranchId());
	}
	...
	private void sendQuery(LabQueryMsg queryMsg) throws IOException {
		...
		if (!CollectionUtil.isCollectionEmpty(ordersList)) {
			...
			if (!machine.getIsPanelOrder()) {
				...
				finalTestSelctionList = machineTestService.getMachineTests(requesterTestList, machine);

				String SpecimenDescriptor = "";

				SpecimenDescriptor = getResultMappingList(analysisList, finalTestSelctionList, analysisListMapped,
						SpecimenDescriptor);

				if (analysisListMapped.size() != 0) {
					container = new Container(queryMsg.specimenIds, analysisListMapped, queryMsg.specimenPosition,
					...
					Patient patient = new Patient(patientFirstName, patientLastName, patientLastName, "", patientId,
				...
				if (!CollectionUtil.isArrayEmpty(requesterTestList.toArray())) {
					...
					case BECHMAN_COULTER_800_DXH:
						...
						machineOrderService.updateAllOrdersIsSentToMachine(ordersList.get(0).getBarcode(),
								ordersList.get(0).getPanelCode(), true, machine.getTenantId(), machine.getBranchId());
	...
	private void SendNegativeQuery(LabQueryMsg queryMsg, List<LabMessages.Analysis> analysisListMapped,
			String SpecimenDescriptor) {
		LabOrderMsg orderMsg;
		Container container;
		container = new Container(queryMsg.specimenIds, analysisListMapped, queryMsg.specimenPosition, null, null,
				SpecimenDescriptor, "", "");

		setPatientInformation(true);

		Patient patient = new Patient(patientFirstName, patientLastName, patientLastName, "", patientId, dateOfBirth,
				gender, attendingPhysician);
		Order order = new Order(patient, container, machine.getPassword(), machine.getName(), null, -1);
		orderMsg = new LabOrderMsg(order);
		conf.recipient.tell(orderMsg, self());
	}
	...
	private LabOrderMsg createMessageSelectionForPanel(int index) {
		...
		LabMessages.Analysis analysis = new Analysis(ordersList.get(index).getPanelCode(), "", "", null);
		analysisList = new ArrayList<LabMessages.Analysis>();
		analysisList.add(analysis);

		container = new Container(ordersList.get(index).getBarcode(), analysisList, "", ordersList.get(0).getPriority(),
				DateUtil.formatDate((ordersList.get(0).getSpecimenCollectionDateAndTime()), "dd/MM/yyyy HH:mm:ss"),
				SpecimenDescriptor, CodesEnumAll.NEW.getValue(), CodesEnumAll.ReportTypeQ.getValue().toString());
	...
	private String getUniversalTestID(Machine machine, String panelName) {
		...
		String panelHostCode = machineTypePanelService.getPanelHostCode(machine.getMachineType().getRid(), panelName);
	...
	private void sendEmptyTS(LabQueryMsg queryMsg) {
		...
		Order order = new Order(patient, container, machine.getPassword(), machine.getName(), "", -1);
		orderMsg = new LabOrderMsg(order);
		conf.recipient.tell(orderMsg, self());
	}
	...
	private String getResultMappingList(List<LabMessages.Analysis> analysisList, List<MachineTest> machineTestList,
			...
			for (MachineTest machineTest : machineTestList) {
				if (machineTest.getTestCatalog().getRequesterTestCode().equals(analysis.code)
					...
					if (machineTest.getTestCatalog().getSpecimenType() != null) {
						SpecimenDescriptor = machineTest.getTestCatalog().getSpecimenType().getCode();
					}
					testMapExist = true;
	...
	private List<String> getRequesterList(LabQueryMsg queryMsg, List<LabMessages.Analysis> analysisList) {
		Result result;
		...
		for (MachineOrder orderInfo : ordersList) {
			...
			if (!machine.getIsPanelOrder()) {
				analysis = new Analysis(orderInfo.getPanelCode(), orderInfo.getTestCode().toString(), "", "", result);
				// Add the test code for the order in a list
				machineQuery = mQueryService.getQueryBySampleAndMachine(queryMsg.specimenIds.toString(), machine);
				analysisList.add(analysis);
				...
				analysis = new Analysis(orderInfo.getPanelCode(), orderInfo.getPanelCode().toString(), "", "", result);
				// Add the required test codes in a list of strings
	...
	private List<MachineOrder> getAllBatchedOrders(LabQueryMsg queryMsg) {
		...
		machine = machineService.getMachineByActorPath(getContext().parent().toString());
		lstMachineTypePanel = machineTypePanelService.getPanelByMachineType(machine.getMachineType().getRid());

		lstOrder = machineOrderService.getAllOrdersByPanelName(lstMachineTypePanel, machine.getTenantId(),
				machine.getBranchId());
	...
	private List<MachineOrder> getMachineOrders(LabQueryMsg queryMsg) {
		...
		// the both conditions
		if (machine.getRequestTestWithNoResultOnly() == true || machine.getRequestNewTestOnly() == true) {
			lstOrder = machineOrderService.getOrderInformationBySampleNoWithOptions(queryMsg.specimenIds.toString(),
					machine.getRequestTestWithNoResultOnly(), machine.getRequestNewTestOnly());
		} else {

			lstOrder = machineOrderService.getOrderInformationBySampleNo(queryMsg.specimenIds.toString(),
	...
	private List<MachineOrder> getPanelOrder(LabQueryMsg queryMsg, MachineType machineType) {
		...
		for (int index = 0; index < machineTypePanelList.size(); index++) {
			MachineOrder newOrder = machineOrderService.getOrderInformationByPanelName(queryMsg.specimenIds.toString(),
					machine.getTenantId(), machine.getBranchId(), machineTypePanelList.get(index).getPanelHostCode());
	...
	private String executePOST(String requestBody, String targetUrl, String API_KEY) throws IOException {
		HttpURLConnection connection = null;
