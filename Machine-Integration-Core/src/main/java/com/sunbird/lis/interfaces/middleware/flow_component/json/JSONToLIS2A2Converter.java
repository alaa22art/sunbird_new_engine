...
public class JSONToLIS2A2Converter extends FlowComponent<RecipientConf> {
	...
	{
		//Activate Order by Order Action ID in Case of succuss 
		...
		if (httpRequstTransObject.getRequest_result_type().equals(REQUEST_RESULT_TYPE.SUCCUSS) 
				&& httpRequstTransObject.getMSG_Type().equals("DFT_P03")) 
		{
			if(!httpRequstTransObject.getPostDetailFinancialTransaction().getTransactionType().equals("DC")
			||!httpRequstTransObject.getPostDetailFinancialTransaction().getTransactionType().equals("CR")
			||!httpRequstTransObject.getPostDetailFinancialTransaction().getTransactionType().equals("CO"))
				
				//Check if order date is today
				
				if (isSameDayOrder(httpRequstTransObject.getPostDetailFinancialTransaction().getTransactionDate()))	
				{
					activatePendingOrtder(httpRequstTransObject.getPostDetailFinancialTransaction()
							.getServSection(), httpRequstTransObject.getPostDetailFinancialTransaction().getRCMOrderActionID());
				}
		}
			...
			// throw new Exception("Error_: ACTIVATE ORDER NOT SUCCUSS");// TODO: handle exception
		}finally
		{
