...
public class SchedulerServer extends FlowComponent<SchedulerServerConf> {
	...
	protected void init() throws Exception {
		...
		tcp.tell(TcpMessage.bind(self(), new InetSocketAddress(machine.getServerIpAddress(), machine.getServerPort()), 5), self());
	...
	protected PartialFunction<Object, BoxedUnit> getBehaviour() {
		return ReceiveBuilder
								.match(Bound.class, msg ->
									{
										...
										ActorRef currentActorConnection = ActorRefConnectionContainer.getRefActor(null).getRef(null);
								...
								.match(CommandFailed.class, msg ->
									...
									{
										throw new RuntimeException("Unable to start server on " + conf.address + ":" + conf.portNumber
												+ " - " + msg.cmd().failureMessage());
									})
								.match(Connected.class, msg ->
									{
										...
										if (checkMachineIpAddress.equals("1")) {
											if (!remoteIpAddress.equals(machine.getMachineIpAddress().toString())) {
												return;
											}
										}
	...
	private PartialFunction<Object, BoxedUnit> getConnectedState(ActorRef connection) {
		return ReceiveBuilder
								...
								.match(Received.class, msg -> conf.recipientActor.tell(new BytesMessage(msg.data().toArray()), self()))
								.match(CommandFailed.class, msg ->
									{
