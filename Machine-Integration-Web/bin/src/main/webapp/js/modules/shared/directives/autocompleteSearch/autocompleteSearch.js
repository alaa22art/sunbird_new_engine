define(['app', 'util', 'config'], function (app, util, config) {
    ...
    //options.required: A boolean or an object of type boolean [OPTIONAL].default is false
    ...
    app.directive('autocompleteSearch', ["$timeout", function ($timeout) {
        return {
            ...
            controller: ['$scope', '$filter', function ($scope, $filter) {
                ...
                $scope.options.required = $scope.options.required != null ? $scope.options.required : false;
                    ...
                    return $scope.options.service(filterablePageRequest)
                        .then(function (response) {
                            ...
                            for (var i = 1; i < $scope.autocompleteData.length; i++) {
                                ...
                                for (var j = 0; $scope.options.dynamicLang && !item.autocompleteCode && j < util.languages.length; j++) {
                                    var langKey = util.languages[j].language;
                                    item.autocompleteCode = util.getDeepValueInObj(item, $scope.options.skeleton.code + "." + langKey);
                                }

                                item.autocompleteDescription = util.getDeepValueInObj(item, descKey);
                                for (var j = 0; $scope.options.dynamicLang && !item.autocompleteDescription && j < util.languages.length; j++) {
                                    var langKey = util.languages[j].language;
                                    item.autocompleteDescription = util.getDeepValueInObj(item, $scope.options.skeleton.description + "." + langKey);
                                }
