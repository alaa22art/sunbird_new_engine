define(['app', 'config', 'util', 'commonData'], function (app, config, util, commonData) {
	...
	app.directive('languageSwitcher', function () {
		return {
			...
			controller: ['$scope', 'tenantMessagesService', '$translate', '$rootScope',
				function ($scope, tenantMessagesService, $translate, $rootScope) {
					$scope.languages = [];
					tenantMessagesService.getSupportedLanguages().then(function (response) {
					...
					$scope.onChange = function () {
						...
						$rootScope.userLocale = util.userLocale
