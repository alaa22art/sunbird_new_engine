define(['app', 'config', 'util', 'commonData'], function (app, config, util, commonData) {
	...
	app.controller('patientSampleLookupCtrl', [
		...
		function ($scope,
			...
			$scope.sampleBarcode = $stateParams.params ? $stateParams.params.barcode : null;
			$scope.selectedMachineOrder = null;
			$scope.dilutionRatio = null;
				...
				transport: {
					read: function (e) {
						...
						if (e.data.id && expandedObj) {
							...
							//TODO: use getResultPatientSampleLookupData
				...
				columns: [
					...
					},
					{
						...
						template: function (dataItem) {
							...
							if (dataItem.transients.patientAge) {
					...
					},
					{
						field: "results",
						width: 200,
						title: util.systemMessages.results,
						template: function (dataItem) {
							if (dataItem.parentRid) {
								return "";
							}
							return dataItem.transients.resultsCount != null ? dataItem.transients.resultsCount : "";
						}
					},
					{
						field: "code",
					...
					},
					{
						field: "queried",
						width: 200,
						title: util.systemMessages.queried,
						template: function (dataItem) {
							return dataItem.transients.queried ? $filter("dateTimeFormat")(dataItem.transients.queried) : "";
						}
					}
				],
			...
			$scope.cancel = function () {
				messegesTransactionLogService.cancelMachineOrder($scope.selectedMachineOrder.rid).then(function () {
			...
			$scope.reorder = function () {
				messegesTransactionLogService.reorderMachineOrder($scope.selectedMachineOrder.rid).then(function () {
					util.createToast(util.systemMessages.successTransaction, "success");
					$scope.refreshOrdersGrid();
				});
			};
