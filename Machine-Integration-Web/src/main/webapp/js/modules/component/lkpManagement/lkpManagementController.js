    ...
    app.controller('lkpManagementCtrl', [
        ...
        function (
            ...
            $scope.highlightSelected = function (lkp, isAdd) {
                if (isAdd) {
                    $("#lkp_" + lkp.rid).closest("md-list-item").addClass("text-highlight");
                } else {
                    $("#lkp_" + lkp.rid).closest("md-list-item").removeClass("text-highlight");
                }
            }
            ...
            $scope.addLkp = function () {
                ...
                dataSource.add(selectedLkp);
                var data = dataSource.data();
                var dataItem = data[data.length - 1];
                $scope.editLkp(dataItem);
            };

            $scope.editLkp = function (dataItem) {
                $("#lkpsGrid").data("kendoGrid").editRow(dataItem);
                $("#lkpsGrid").data("kendoGrid").select("tr[data-uid=" + dataItem.uid + "]");
                $scope.lkpChanged = true;
            };

            $scope.deleteLkp = function () {
                dataSource.remove($scope.selectedLkp);
                dataSource.sync();
                $scope.selectedLkp = null;
            };
