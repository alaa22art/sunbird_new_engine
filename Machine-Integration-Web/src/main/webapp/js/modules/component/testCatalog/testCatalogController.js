    ...
    app.controller('testCatalogCtrl', [
        ...
        function (
            ...
            /* $scope.saveTest = function () {
                 testCatalogService.saveTest($scope.selectedMachine).then(function (response) {
            ...
            $scope.saveTest = function () {
                ...
                if ($scope.selectedTest.id == "") {
                    testCatalogService.addTest($scope.selectedTest).then(function (response) {
                        util.createToast(util.systemMessages.success, "success");
                        $scope.selectedTestChanged = false;
                        $scope.selectedTest = null;
                        $scope.clearFields();
                    ...
                    testCatalogService.updateTest($scope.selectedTest).then(function (response) {
                        util.createToast(util.systemMessages.success, "success");
                        $scope.selectedTestChanged = false;
                        $scope.selectedTest = null;
                        $scope.clearFields();
            ...
            var dataSource = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        testCatalogService.getTestCatalogPage(e).then(function (data) {
                            e.success(data);
                        });
                    }
                },
            ...
            $scope.testCatalogeGridOptions = {
                columns: [{
                    field: "name",
                    title: util.systemMessages.TestCatalageGridTestName
                },
                {
                    field: "requesterTestCode",
                    title: util.systemMessages.TestCatalageGridRequesterCode
                },
                {
                    field: "specimenType",
                    filterable: {
            ...
            $scope.deleteTest = function () {
                if ($scope.selectedTest.rid != null) {
                    testCatalogService.getTestById($scope.selectedTest.rid).then(function (responce) {
                        $scope.selectedTest = responce.data;
                        testCatalogService.deleteAllTest($scope.selectedTest).then(function (responce) {
                            util.createToast(util.systemMessages.success, "success")
                            $scope.clearFields();
                            $scope.refreshGrid();
                        });
