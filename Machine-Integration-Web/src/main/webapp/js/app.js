define(['config', 'routes', 'angular', 'util', 'commonData'], function (config, routes, angular, util, commonData) {
    ...
    ]).run(["$window", "$timeout", "$translate", "$rootScope", "$mdToast", "$transitions", "$http", "$q", "$cookies", "$state", "$filter", "socketService",
        function ($window, $timeout, $translate, $rootScope, $mdToast, $transitions, $http, $q, $cookies, $state, $filter, socketService) {
            ...
            util.$filter = $filter;
            util.getUserData();
            if (util.isLoggedIn() && util.user.branchId != null) {
